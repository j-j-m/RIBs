...
public protocol LaunchRouting: ViewableRouting {
    ...
    func launch(from window: PlatformWindow)
}
...
open class LaunchRouter<InteractorType, ViewControllerType>: ViewableRouter<InteractorType, ViewControllerType>, LaunchRouting {
    ...
    public final func launch(from window: PlatformWindow) {
        
        #if os(iOS)
        window.rootViewController = viewControllable.controller
        window.makeKeyAndVisible()
        #else
        window.contentViewController = viewControllable.controller
        window.makeKey()
        #endif
